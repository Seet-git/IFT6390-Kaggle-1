...
from src.preprocessing import remove_stopwords, remove_low_high_frequency, lemmatise, steeming
...
def k_fold_cross_validation(hp, k=5):
    ...
    X_train, X_test = remove_low_high_frequency(low_threshold=hp.low_threshold,
                                                high_threshold=hp.high_threshold, X_train=X_train, X_test=X_test)
    ...
    print(f"Mean F1-score across all folds: {mean_k_fold_accuracy}")
